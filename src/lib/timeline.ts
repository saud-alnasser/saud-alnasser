...
// out in time: the home page's education section, the CV's education
// section, the dist check's reading-order expectation, and the browser tests
// all read this rather than each keeping a copy of the rule. The home page's
// timeline from study to work is built on it, by `journey` at the foot of
// this file.
//
// The rule: the institutions newest first, with one node for the
// online-courses phase immediately after the most recent institution. By
// position rather than by date, because the Code with Mosh completions carry
// no date and a phase that spans years needs none to sit where it belongs;
// so the record reads university, online courses, high school whatever dates
// the courses carry (src/content/README.md, "education/"). The node counts
// every course and runs from the earliest dated one to the latest, found
// here whatever order the courses arrive in. A date is YYYY, YYYY-MM, or
// YYYY-MM-DD, and those compare as text: a shorter one is a prefix of the
// longer dates in its span and so sorts first, which is where
// src/lib/order.ts puts it too.
//
...
// `education` arrives oldest first, by its start; the items are laid out in
// that direction, the node before the most recent institution, and then read
// the other way.
export function educationTimeline<Entry, Course extends { date?: string | number }>(
  ...
  if (courses.length === 0) return items.reverse();
...
// exactly as it is built for every other output, turned oldest first so each
// experience entry can be laid in by when it started, and the whole read
// newest first again.
//
...
export function journey<Education, Experience, Course extends { date?: string | number }>(
  ...
  const items: JourneyItem<Education, Experience>[] = educationTimeline(education, courses).reverse();
