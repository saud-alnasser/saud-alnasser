...
3. **The GitHub address leaves both documents.** Neither the contact line nor anything else on the CV or the resume writes a profile address out as text. A project's repository link on the CV, which begins with the GitHub profile address because the repositories are under that account, is the project's address rather than the profile's and stays where [[efforts/5-sections-and-resume/spec]] put it; a check reads a profile address as written out only where nothing continues it as a path. (Clarified on 2026-09-22 at review, when the first wording read as forbidding the project links too.) The code is the one profile on paper; every profile still reaches both JSON Resume documents, the home page, and the README. Saud's decision on 2026-09-22, and it retires the rule the component has carried since the code arrived, that a profile which is not the code is written out so that no fact is dropped in silence: this drop is decided rather than silent, and the fact survives at `basics.profiles[].url`.
...
3. `[data-cv-contact]` on all four document pages contains no `github.com` and no address of any profile `profile.yaml` lists, and `pdftotext` over the four PDFs and the filled copies prints none of them as its own item, with or without its scheme, with or without a trailing slash or a query, while a project's repository link that continues the GitHub address as a path does not count. Asserted from the content's profiles list rather than from a literal, so a third profile added later is refused the same way.
