...
1. **Newest first, everywhere.** Every list the site or a document lays out in time reads newest first: the education section's studies timeline, the CV's education section, and the resume's education section. The online-courses node keeps its place relative to the institutions under the existing position rule, read in the new direction, so on today's content the university comes first and the online courses after it. The rule stays one rule in `src/lib/timeline.ts`, read by every output and every check.
...
4. **A header that stays and says where you are.** On every page the header stays on screen as the page scrolls. On the home page it links to each of the five sections, and the link for the section being read is marked current, in a way that does not rest on colour alone, and exposed to assistive technology. Pressing a section link brings that section's heading to the top of the viewport, just below the header, with nothing hidden under the header. On the CV and resume pages the same links lead to the sections on the home page.
...
8. Each of the four old addresses, fetched from the built output, carries a meta refresh to its home-page section in the same language and a canonical link to the same place. It has no script. A browser test opening each old address with each old anchor ends on the home page with the named heading in view. The sitemap has no work or education page.
