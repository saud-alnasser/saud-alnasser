...
| `src/content/` | **the content source**: one YAML file per entry under `projects/`, `experience/`, `education/`, `certificates/`, `skills/`, `languages/`, and `profile.yaml`, plus `self-study.yaml`, the providers and topics of the one self-study entry the CV reads the courses as, with the certificate documents and their previews under `certificates/files/` and the degree certificate with its preview under `education/files/`. Every fact the site or the CV shows lives here and nowhere else; `src/content/README.md` documents the format |
| `src/content.config.ts` | the content contract: the Zod schema of each collection, which the build enforces |
| `src/pages/`, `src/layouts/`, `src/components/` | the Astro templates: pages under `[locale]/` for `en` and `ar`, the `resume.json` and `robots.txt` endpoints, one base layout, one component per entry type, and the pieces they share: the inline icon, the technology badge, the QR code the documents carry the LinkedIn address as (the one profile on either document since 2026-09-22; which profile is the code is `codedProfile` in `src/lib/networks.ts`), the card, the fold, the two header controls, and the phone menu; the home page's own pieces, the stat tile and the featured project card; the education timeline's online-courses node, which opens to list the courses (`CourseList.astro`), and the page the two old addresses serve, forwarding to their sections (`Forward.astro`). The home page is the whole portfolio, one section each for about, experience, projects, education, and skills |
| `src/lib/` | UI strings per locale with the plural helper (`i18n.ts`), the language fallback and its gap report (`localized.ts`), the entry ordering (`order.ts`), the education timeline, newest first with the online-courses node in its place, decided once for the home page, the CV, and the checks (`timeline.ts`), the home page's five sections the header links to (`sections.ts`), the old addresses that forward to a section (`forwards.ts`), whether an entry appears at all and which document it belongs to (`shown.ts`), the one line a language's level prints as, score and year included, on the documents and in `fluency` (`languages.ts`), the JSON Resume mapper (`resume.ts`), the base-path join every published path goes through, and the repository address the footer links as the source (`paths.ts`), a certificate's PDF and preview as the page's dialog reads them, for the certification cards and the course rows alike (`certificates.ts`), the icon a profile's network carries and which profile is the documents' code (`networks.ts`), the icon bodies themselves (`icons.ts`), which the icon component and the QR code both draw from, and which technology names carry a Simple Icons mark and which logo credits the footer owes (`technologies.ts`) |
...
| `tests/` | the Playwright tests over the built site, and the three Node tests `pnpm test:content` runs first: the technology marks' licences, the education timeline's order, and the skill keywords' backing, which holds each keyword to a shown project, a self-study topic, or a line in its own table (`skills.test.mjs`) |
