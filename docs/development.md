...
pnpm test:content # the Node tests of the technology marks' licences, the home timeline's order, and every skill keyword's backing, then adds a temporary project, course, certification, and language, rebuilds, and checks each shows everywhere it should and nowhere else; then tries the featured mark on a hidden project, on a second project, and on none
...
a script, an image, or anything else it loads, naming the file and the address. A link a visitor
